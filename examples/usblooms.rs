...
use anyhow::Result;
use illumos_devinfo as devinfo;
...
impl UsbDevice {
    fn from_node(n: &devinfo::Node) -> Option<UsbDevice> {
        ...
        let vendor_name: String = usbvendname?.as_cstr()?.to_str().ok()?.to_string();
        let product_name: String = usbprodname?.as_cstr()?.to_str().ok()?.to_string();
...
fn main() -> Result<()> {
    ...
    let mut looms: HashMap<String, HashMap<String, UsbDevice>> = Default::default();
